...
class AgileStoreLocator_Public {
	...
	*/
	...
	public function searchBox($atts) {
		...
		wp_enqueue_style( $this->AgileStoreLocator.'-asl-search',  ASL_URL_PATH.'public/css/asl_search.css', array(), $this->version, 'all' );
		...
		wp_enqueue_script( $this->AgileStoreLocator.'-search', ASL_URL_PATH . 'public/js/asl_search.js', array('jquery'), $this->version, true );
	...
	public function frontendStoreLocator($atts) {
		...
		if(isset($atts['mobile_stores_limit']) && !is_nan($atts['mobile_stores_limit'])) {
		...
		*/
		...
		wp_localize_script( $this->AgileStoreLocator.'-script', 'asl_attributes',$all_attributes);
	...
	private function add_test_stores() {
		...
		$content = file_get_contents('/home/dev/projects/wordpress_language/jyhem.json');
