...
class AgileStoreLocator_Admin {
  ...
  private function _get_custom_fields() {
	...
	public function add_new_store() {
		...
		//	Custom Field
		$custom_fields = stripslashes_deep($_REQUEST['asl-custom']);
	...
	public function add_attribute() {
		...
		if($value && $wpdb->insert(ASL_PREFIX.$table, array('name' => $value)))
		{
			$response->msg = $title." Added Successfully";
	 			$response->success = true;
		}
		else
		{
	...
	public function update_attribute() {
		...
		if($at_id & $value && $wpdb->update(ASL_PREFIX.$table, array('name' => $value), array('id' => $at_id)))
		{
	...
	public function get_attributes() {
		...
		foreach($data_output as $aRow)
	    {
	    	...
	    	$row->action = '<div class="edit-options"><a data-value="'.$row->name.'" data-id="'.$row->id.'" title="Edit" class="edit_attr"><svg width="14" height="14"><use xlink:href="#i-edit"></use></svg></a><a title="Delete" data-id="'.$row->id.'" class="delete_attr g-trash"><svg width="14" height="14"><use xlink:href="#i-trash"></use></svg></a></div>';
	...
	public function import_store() {
		...
		*/
		...
		$header_columns = ['title', 'description', 'street', 'city', 'state', 'postal_code', 'country', 'lat', 'lng', 'phone', 'fax', 'email', 'website', 'is_disabled', 'logo', 'categories', 'marker_id', 'description_2', 'open_hours', 'order', 'brand'];
		...
		foreach($rows as $t) {
			...
			if(isset($t['logo_image']) && filter_var(filter_var($t['logo_image'], FILTER_SANITIZE_URL), FILTER_VALIDATE_URL) !== false) {
				...
				if(in_array($extension, ['jpg', 'png', 'gif', 'svg', 'jpeg'])) {
					...
					file_put_contents($file_path, file_get_contents($t['logo_image']));	

					$t['logo_image'] = $file_name;
				}
			}
			else
			if(isset($t['logo_name']) && $t['logo_name'] != '') {

				$t['logo_name'] = trim($t['logo_name']);
				$logoresult = $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."storelogos WHERE name = '".$t['logo_name']."'" );
			...
			if($t['brand'] != '') {
				...
				foreach ($brands as $brand) {
					...
					$brand 					= trim($brand);
					$brand_in_db 		= $wpdb->get_results("SELECT * FROM ".ASL_PREFIX."brands WHERE name = '".$brand."'" );
			...
			if($create_category && $t['categories'] != '') {
				...
				foreach ($categorys as $_cat) {
					...
					$_cat = iconv("UTF-8", "ISO-8859-1", trim($_cat));
			...
			if($t['title'] != '') {
				...
				//Get the ID
				$store_id = ($is_update)?$t['update_id']:$wpdb->insert_id;
				...
				if($logoid == '0') {
					...
					if(isset($t['logo_image'])) {
						...
						$response->summary[] = 'Row: '.$index.", logo ".$t['logo_name']." not found";
				}
			}
  ...
  public function export_store() {
    ...
    //	Header titles
    $headers = ['title', 'description', 'street', 'city', 'state', 'postal_code', 'country', 'lat', 'lng', 'phone', 'fax', 'email', 'website', 'is_disabled', 'logo', 'categories', 'marker_id', 'description_2', 'open_hours', 'order', 'brand'];
    ...
    foreach ($stores as $value) {
    	...
    	if(isset($value->custom) && $value->custom) {
    		...
    		foreach ($fields as $field => $f_value) {
					...
					if(isset($custom_fields_data[$field]))
						$value->$field = $custom_fields_data[$field];
				}
	...
	public function get_stat_by_month() {
		...
		if($stats_type == 'chart') {
			...
			$limit   = (isset($_REQUEST['len']) && !is_nan($_REQUEST['len']))?$_REQUEST['len']: '10';
	...
	public function save_custom_fields() {
		...
		$data_params = array('content' => json_encode($fields), 'type'=> 'fields');
