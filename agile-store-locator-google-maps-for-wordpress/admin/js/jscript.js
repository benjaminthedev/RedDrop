  ...
  app_engine.uploader = function($form, _URL, _done /*,_submit_callback*/ ) {
    ...
    $form.fileupload({
        ...
        add: function(e, _data) {
          ...
          if (['jpg', 'png', 'jpeg', 'gif', 'JPG', 'svg', 'zip', 'csv'].indexOf(exten) == -1) {
  ...
  app_engine['pages'] = {
    ...
    manage_attribute: function(_params) {
      ...
      table = $('#tbl_attribute').dataTable({
        ...
        "iDisplayLength": 10,
        "sAjaxSource": ASL_REMOTE.URL + "?action=asl_get_attributes&type=" + _params.name,
        "columns": [
          { "data": "check" },
          { "data": "id" },
          { "data": "name" },
      ...
      $('#btn-asl-new-attr').bind('click', function(e) {
        ...
        aswal({
            ...
            text: "Would like to add new " + _params.title + "?",
            input: 'text',
            type: "question",
          ...
          .then(function(result) {
            ...
            if (result) {
              ...
              ServerCall(ASL_REMOTE.URL + "?action=asl_add_attribute", { title: _params.title, name: _params.name, value: result }, function(_response) {
      ...
      $('#btn-asl-delete-all').bind('click', function(e) {
        ...
        aswal({
          ...
          ServerCall(ASL_REMOTE.URL + "?action=asl_delete_attribute", { title: _params.title, name: _params.name, item_ids: item_ids, multiple: true }, function(_response) {
        ...
        aswal({
          ...
          .then(function(result) {
            ...
            if (result) {
              ...
              ServerCall(ASL_REMOTE.URL + "?action=asl_update_attribute", { id: _id, title: _params.title, name: _params.name, value: result }, function(_response) {
      ...
      $('#tbl_attribute tbody').on('click', '.delete_attr', function(e) {
        ...
        aswal({
          ...
          function() {
            ...
            ServerCall(ASL_REMOTE.URL + "?action=asl_delete_attribute", { title: _params.title, name: _params.name, category_id: _category_id }, function(_response) {
    ...
    manage_categories: function() {
      ...
      $('#btn-asl-update-categories').click(function() {
        ...
        if ($("#update_category_icon").attr("data-id") == "same") {
          ...
          ServerCall(ASL_REMOTE.URL + "?action=asl_update_category", { data: { category_id: $("#update_category_id_input").val(), action: "same", category_name: $("#update_category_name").val(), "ordr": $("#update_category_ordr").val()  } },
            function(_response) {
    ...
    dashboard: function() {
      ...
      function updateChart(m, y) {
        ...
        ServerCall(ASL_REMOTE.URL + "?action=asl_get_stats&stat_type=chart", { m: m, y: y }, function(_response) {
       ...
       * @return {[type]}   [description]
       */
      function getViews(m, y) {

        var rows_len = $('#asl-search-len').val();

        ServerCall(ASL_REMOTE.URL + "?action=asl_get_stats&stat_type=views", { m: m, y: y, len: rows_len }, function(_response) {

          var stores_views = _response.stores;
          var search_views = _response.searches;

          //  Clear old records
          jQuery('#asl-stores-view li:not(:first-child)').remove();
          jQuery('#asl-searches-view li:not(:first-child)').remove();
          ...
          if (stores_views && stores_views.length) {
            ...
            for (var s = 0; s < stores_views.length; s++) {
              ...
              stores_views_html += `<li class="list-group-item">
                            <div class="row">
                              <div class="col-2">${_store_view.store_id}</div>
                              <div class="col-8">${_store_view.title}</div>
                              <div class="col-2">${_store_view.views}</div>
                            </div>
                          </li>`;
            }
          } else {
          ...
          jQuery('#asl-stores-view').append(stores_views_html);
          ...
          if (search_views && search_views.length) {
            ...
            for (var s = 0; s < search_views.length; s++) {
              ...
              searches_views_html += `<li class="list-group-item">
                            <div class="row">
                              <div class="col-8">${_store_view.search_str}</div>
                              <div class="col-4">${_store_view.views}</div>
                            </div>
                          </li>`;
            }
          } else {
          ...
          jQuery('#asl-searches-view').append(searches_views_html);
      ...
      $('#asl-search-view,#asl-search-len').bind('change', function(e) {
        ...
        var temp = $('#asl-search-view').val().split('-'),
          m = temp[0],
          y = temp[1];
      ...
      temp = $('#asl-search-view')[0].value.split('-');
    ...
    user_setting: function(_configs) {
      ...
      for (var i in _keys) {
        ...
        if ( /*_keys[i] == 'layout' || */ _keys[i] == 'map_layout' || _keys[i] == 'infobox_layout' || _keys[i] == 'color_scheme' || _keys[i] == 'color_scheme_1' || _keys[i] == 'color_scheme_2' || _keys[i] == 'color_scheme_3' || _keys[i] == 'font_color_scheme') {
      ...
      $('#btn-asl-save-schema').on('click', function(e) {
        ...
        console.log('===> jscript.js ===> custom_fields', custom_fields);
