...
if (! function_exists('mfn_builder_wrap')) {
	...
	function mfn_builder_wrap($wrap = false, $parent_id = false, $uids = false)
	{
		...
		echo '<div class="mfn-element mfn-wrap '. esc_attr($class) .'" data-size="'. esc_attr($sizes[$size]) .'" data-title="'. esc_html__('Wrap', 'mfn-opts') .'">';
...
if (! function_exists('mfn_builder_item')) {
	...
	function mfn_builder_item($item_type, $item = false, $parent_id = false, $uids = false)
	{
		...
		echo '<div class="mfn-element mfn-item mfn-item-'. esc_attr($item_std['type']) .'" data-size="'. esc_attr($sizes[$item_std['size']]) .'" data-title="'. esc_attr($item_std['title']) .'">';
...
if (! function_exists('mfn_get_sliders')) {
	...
	function mfn_get_sliders()
	{
		...
		if (function_exists('rev_slider_shortcode')) {
			...
			$array = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$table_name` WHERE `type` != %s ORDER BY `title` ASC", 'template'));
...
if (! function_exists('mfn_get_sliders_layer')) {
	...
	function mfn_get_sliders_layer()
	{
		...
		if (function_exists('layerslider')) {
			...
			$array = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$table_name` WHERE `flag_hidden` = %d AND `flag_deleted` = %d ORDER BY `name` ASC", 0, 0));
...
add_action('admin_enqueue_scripts', 'mfn_builder_enqueue');
